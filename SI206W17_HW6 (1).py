...
def personal_map(function,lst):
    new_lst = []
    for x in lst:
        new_lst.append(function(x))
    return new_lst
...
programmers = [Student(a,b,c) for a,b,c in student_tups_list]
...
def len_check(gen):
    return (x for x in gen if len(x)>40)
# Define main_filterer
def main_filterer(names):
    return len_check(readfiles(names))
...
provided_file_names = ["samplehw6_2.txt","samplehw6_1.txt"]
